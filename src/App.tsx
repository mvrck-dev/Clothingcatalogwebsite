...
export default function App() {
    ...
    const uniqueValues = useMemo(() => {
        const getUnique = (key: keyof Product) =>
            Array.from(new Set(products.map(p => p[key]?.toString()).filter(Boolean))).sort() as string[];
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
            {/* Header */}
            <header className="bg-gradient-to-r from-purple-600 to-purple-800 text-white px-4 md:px-8 py-6 shadow-lg sticky top-0 z-50">
                <div className="flex items-center justify-between mb-6">
                    {/* Hamburger menu for mobile */}
                    <button
                        onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)}
                        className="md:hidden text-white p-2 hover:bg-white/20 rounded transition"
                    ...
                    <h1 className="flex-1 text-center" style={{ fontFamily: 'cursive', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 'bold', textShadow: '2px 2px 4px rgba(0,0,0,0.2)' }}>
                        Auralane
                    </h1>
                    <div className="flex gap-2 bg-white/20 p-1 md:p-2 rounded-lg">
                        <button
                            onClick={() => setLang('en')}
                            className={`px-2 md:px-4 py-1 md:py-2 rounded transition text-sm md:text-base ${lang === 'en' ? 'bg-white text-purple-600' : 'bg-white/30 text-white'
                                }`}
                        >
                            EN
                        </button>
                        <button
                            onClick={() => setLang('fi')}
                            className={`px-2 md:px-4 py-1 md:py-2 rounded transition text-sm md:text-base ${lang === 'fi' ? 'bg-white text-purple-600' : 'bg-white/30 text-white'
                                }`}
                        >
                            FI
                        </button>
                ...
                <div className="flex flex-col md:flex-row gap-4 max-w-4xl mx-auto">
                    <button
                        ...
                        className="bg-white text-purple-600 px-6 py-3 rounded-lg hover:bg-gray-100 transition whitespace-nowrap hidden md:block"
                    >
                        {t.browse}
                    </button>
                    <div className="flex-1 flex bg-white rounded-lg overflow-hidden shadow">
                        <input
                            ...
                            className="flex-1 px-4 py-3 outline-none text-gray-900 text-sm md:text-base"
                        />
                        <button className="bg-purple-600 text-white px-4 md:px-6 hover:bg-purple-700 transition text-sm md:text-base">
                            {t.search}
                        </button>
                    </div>
                </div>
            ...
            w-[280px] md:w-[300px] bg-white shadow-lg p-6 overflow-y-auto
            z-50 md:z-auto
            ...
            ${!sidebarOpen && !mobileSidebarOpen ? 'md:hidden' : ''}
          `}
                >
                    ...
                    <div className="md:hidden mb-6 border-b pb-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Menu</h3>
                        <div className="flex flex-col gap-2">
                            <button onClick={() => setCurrentPage('about')} className="text-left px-4 py-2 hover:bg-gray-100 rounded-lg transition text-gray-700 font-medium">
                                {t.aboutUs}
                            </button>
                            <button onClick={() => setShowContactDialog(true)} className="text-left px-4 py-2 hover:bg-gray-100 rounded-lg transition text-gray-700 font-medium">
                                {t.contactUs}
                            </button>
                            <a href="mailto:guruconsultantsfinland@gmail.com" className="text-left px-4 py-2 hover:bg-gray-100 rounded-lg transition text-gray-700 font-medium">
                                {t.reportIssue}
                            </a>
                        </div>
                    </div>
                ...
                <main className="flex-1 p-4 md:p-8">
                    <div className="mb-6">
                        <h2 className="text-gray-600">
                            <span className="text-base md:text-lg">{filteredProducts.length} {t.productsFound}</span>
                        </h2>
                    </div>
                    {loading ? (
            ...
            {/* Footer */}
            <footer className="bg-white border-t mt-16 py-6 md:py-8">
                <div className="max-w-7xl mx-auto px-8 flex flex-col md:flex-row justify-center items-center gap-4 md:gap-8 text-sm md:text-base">
                    <button onClick={() => setCurrentPage('about')} className="text-purple-600 hover:text-purple-800">
                        {t.aboutUs}
                    </button>
                    <button onClick={() => setShowContactDialog(true)} className="text-purple-600 hover:text-purple-800">
            ...
            {/* Contact Dialog */}
            {showContactDialog && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[110]" onClick={() => setShowContactDialog(false)}>
